"""Minimal training script for Lomtev U-Net on BCI-IV data.

Reproduces the FingerFlex (Lomtev, 2023) result on BCI Competition IV
Dataset 4. Target: ~0.7 Pearson r (average across 5 fingers).

Usage:
    python scripts/train_lomtev_bci4.py [--subject 1] [--epochs 20] [--gpu 0]
    python scripts/train_lomtev_bci4.py --subject 1 --epochs 2 --no-wandb
"""
...
from src.data.dataset_lomtev import build_bci4_lomtev_datasets
...
def main():
    ...
    # Data
    print(f"Building BCI-IV pipeline for subject {args.subject}...")
    t0 = time.time()
    ds = build_bci4_lomtev_datasets(args.subject, config)
    ...
    print(f"  Smoothing: val σ={smooth_sigma_val}, test σ={smooth_sigma_test}")
